...
1. Bump all **three** version sites together — `ts/package.json`, `VERSION`
   in `ts/src/ebnf.ts` and `const VERSION` in `go/ebnf.go`. They are held
   equal by `ts/test/version.test.js` and `go/version_test.go`.
...
6. Confirm `npm view @tabnas/ebnf@$V version`, and **query both tags
   exactly**:
