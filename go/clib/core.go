...
//
// libtabnasebnf is the ebnf format parser as a C shared library, one of
...
import (
	...
	plug "github.com/tabnas/ebnf/go"
)

const (
	templateVersion = "v1"
	libName         = "libtabnasebnf"
	formatName      = "ebnf"
	valueOut        = false
)
...
// newParser builds one engine with the ebnf grammar installed
...
func newParser() (parseFn, error) {
	return func(src string) (any, error) { _, err := plug.ParseEbnf(src); return nil, err }, nil
}
...
// parseWith answers whether src is in the ebnf language — and,
