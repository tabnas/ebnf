...
// Package main builds the C-ABI shared library: libtabnasebnf.
//
// tabnas-clib-template: v1 (stamped by admin tasks/adopt-clib.sh;
// edit the template and re-stamp, not this file).
//
//	go build -buildmode=c-shared -o libtabnasebnf.so ./clib
//
// ebnf TEXT IN, VERDICT (AND VALUE) OUT. This is one of the
...
// tabnas_grammar builds a ebnf parser and returns a handle to it.
...
// tabnas_parse checks one input against the ebnf grammar.
//
//export tabnas_parse
