# libtabnasebnf — the ebnf parser as a C ABI

<!-- tabnas-clib-template: v1 — stamped by admin tasks/adopt-clib.sh;
     edit the template and re-stamp, not this file. -->

The ebnf format parser as a C shared library, so languages with no
tabnas port can parse and validate ebnf input. This is one of the
...
| Function | Returns |
|---|---|
| `tabnas_version()` | `{"ok":true,"lib":"libtabnasebnf","format":"ebnf","template":"v1"}` |
...
// link against libtabnasebnf; every call returns a JSON []u8 to free with
// c.tabnas_free.
```
