...
const { gatedDocs, tutorials, WITHHELD } = require('../scripts/gated-docs.cjs')
...
describe('docs-style', () => {
  ...
  // A page can be reader-facing and still be wrong to gate: the whole
  // Go documentation surface here is another repository's, copied and
  // never rewritten, so a clean Vale run over it would certify the wrong
  // content. The debt stays visible -- each entry exists, is excluded,
  // names a reason, and is listed in the guide -- so it cannot go quiet.
  test('withheld-pages-are-recorded-not-forgotten', () => {
    const gated = gatedDocs()
    const guide = Fs.readFileSync(GUIDE, 'utf8')
    const faults = []
    for (const [page, reason] of Object.entries(WITHHELD)) {
      if (!Fs.existsSync(Path.join(REPO, page))) {
        faults.push(`${page}: withheld but not on disk`)
      }
      if (gated.includes(page)) {
        faults.push(`${page}: withheld and gated`)
      }
      if (!reason || reason.length < 10) {
        faults.push(`${page}: no reason recorded`)
      }
      if (!guide.includes(page)) {
        faults.push(`${page}: not listed in docs/STYLE-GUIDE.md`)
      }
    }
    Assert.deepEqual(faults, [],
      `withheld pages (docs/STYLE-GUIDE.md):\n${faults.join('\n')}`)
  })
